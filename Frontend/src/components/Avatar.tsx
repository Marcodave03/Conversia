...
export function Avatar({
  ...
  // Play animation
  useEffect(() => {
    const anim = animation || "Idle"; // fallback to Idle
    ...
    return () => {
      if (actions[anim]) actions[anim].fadeOut(0.5);
    };
  }, [animation, actions]);
  ...
  useFrame(() => {
    ...
    // Facial Expression
    if (expression && nodes.EyeLeft instanceof THREE.SkinnedMesh && nodes.EyeLeft.morphTargetDictionary) {
      const face = facialExpressions[expression as keyof typeof facialExpressions] || {};
    ...
    if (mouthCues.length > 0 && audioDuration > 0) {
      ...
      Object.values(correspondingViseme).forEach((viseme) => {
        lerpMorphTarget(viseme, viseme === activeViseme ? 1 : 0, 0.3);
      });
    }
  });
